...
bool ICACHE_FLASH_ATTR ssd1306_init(uint8_t id)
{
    ...
    // os_free old context (if any)
    ssd1306_term(id);

    ctx = (oled_i2c_ctx *) os_zalloc(sizeof(oled_i2c_ctx));
    if (ctx == NULL)
    {
        os_printf("Alloc OLED context failed.\n");
        goto oled_init_fail;
    }
    if (id == 0)
    {
              ...
              ctx->buffer = (uint8_t *) os_zalloc(1024); // 128 * 64 / 8
              ctx->width = 128;
              ctx->height = 64;
        #elif (PANEL0_TYPE == SSD1306_128x32)
              ctx->type = SSD1306_128x32;
              ctx->buffer = (uint8_t *) os_zalloc(512);  // 128 * 32 / 8
              ctx->width = 128;
              ctx->height = 32;
        #else
          #error "Panel 0 undefined"
              ...
              if (ctx->buffer == NULL)
              {
                  os_printf("Alloc OLED buffer failed.\n");
                  goto oled_init_fail;
              }
    ...
    {
        i2c_stop();
        os_printf("OLED I2C bus not responding.\n");
        goto oled_init_fail;
    }
    i2c_stop();
...
oled_init_fail:
    if (ctx && ctx->buffer) os_free(ctx->buffer);
    if (ctx) os_free(ctx);
    return false;
}
...
void ICACHE_FLASH_ATTR ssd1306_clear(uint8_t id)
{
    ...
    if (ctx->type == SSD1306_128x64)
    {
        os_memset(ctx->buffer, 0, 1024);
    }
    else if (ctx->type == SSD1306_128x32)
    {
        os_memset(ctx->buffer, 0, 512);
    }
...
void ICACHE_FLASH_ATTR ssd1306_update_buffer(uint8_t id, uint8_t* data, uint16_t length)
{
    ...
    if (ctx->type == SSD1306_128x64)
    {
        os_memcpy(ctx->buffer, data, (length < 1024) ? length : 1024);

    }
    else if (ctx->type == SSD1306_128x32)
    {
        os_memcpy(ctx->buffer, data, (length < 512) ? length : 512);
    }
    ctx->refresh_right = ctx->width - 1;
    ctx->refresh_bottom = ctx->height - 1;
    ctx->refresh_top = 0;
